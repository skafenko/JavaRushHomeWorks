...
В классе amy.Solution создайте публичный статический класс AmigoThreadFactory, реализующий интерфейс ThreadFactory
...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory {
        private static AtomicInteger numberFactory = new AtomicInteger(1);
        int currentFactory;
        private AtomicInteger numberThread = new AtomicInteger(1);
        private ThreadGroup threadGroup;

        public AmigoThreadFactory() {
            currentFactory = numberFactory.getAndIncrement();
        ...
        public Thread newThread(Runnable r) {
            ...
            thread.setName(threadGroup.getName() + "-pool-" + currentFactory + "-thread-" + numberThread.getAndIncrement());
            return thread;
        }
    }
